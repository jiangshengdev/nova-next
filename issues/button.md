...
> 处理进度：已在 `src/components/button/nova-button.tsx` 与对外入口中导出 `NovaButtonProps` 类型。
...
> 处理进度：默认插槽已直接返回原始节点，样式改为 `inline-flex + gap`，不再依赖额外 `<span>`。

## 3. 单测缺乏交互覆盖

【已解决】

- 位置：`src/components/button/__tests__/button.behavior.test.tsx`
...
> 处理进度：新增 `button.behavior.test.tsx` 与 `button.environment.test.tsx` 覆盖交互与环境逻辑，原快照文件更名为 `button.basic.test.tsx`。
