...
export function createChannel(params: ChannelParams) {
  const { label, title, value, onInput, onUpdate, onBlur } = params

  return (
    <label class="nova-color-picker-label">
      <div class="nova-color-picker-label-text" title={title} aria-label={title}>
        {label}
      </div>

      <NumberInput value={value.toString()} onInput={onInput} onUpdate={onUpdate} onBlur={onBlur} />
    </label>
  )
}

export function createAlpha(params: AlphaParams) {
